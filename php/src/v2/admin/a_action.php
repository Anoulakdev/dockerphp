...
if (isset($_POST['add'])) {
	...
	$result = $conn->query("SELECT * FROM admin WHERE a_username = '$a_username' AND a_password = '$a_password'");
...
if (isset($_POST['update'])) {
	...
	$query = "UPDATE admin SET a_username=?,a_password=?,a_name=? WHERE a_id=?";
	$stmt = $conn->prepare($query);
	$stmt->bind_param("sssi", $a_username, $a_password, $a_name, $a_id);
	$stmt->execute();
